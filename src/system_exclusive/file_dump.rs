...
impl FileType {
    fn extend_midi(&self, v: &mut Vec<u8>) {
        match self {
            Self::MIDI => v.extend(b"MIDI"),
            Self::MIEX => v.extend(b"MIEX"),
            Self::ESEQ => v.extend(b"ESEQ"),
            Self::TEXT => v.extend(b"TEXT"),
            Self::BIN => v.extend(b"BIN "),
            Self::MAC => v.extend(b"MAC "),
            Self::Custom(chars) => v.extend(chars),
        }
    }
}
